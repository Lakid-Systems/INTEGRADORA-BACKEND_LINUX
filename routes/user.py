...
def read_user(id: int, db: Session = Depends(get_db)):
    db_user = crud.users.get_user_by_usuario(db=db, usuario=id)  # ❌ Corregido error en `id=ID`
    if db_user is None:
...
@user.post("/login/", tags=["User Login"])  # ✅ Quité `response_model=schemas.users.UserLogin`
def read_credentials(usuario: schemas.users.UserLogin, db: Session = Depends(get_db)):
    ...
    token: str = solicita_token(usuario.model_dump())  # ✅ Corregí `usuario.dict()`
    
    return JSONResponse(status_code=200, content={"token": token})  # ✅ Devuelve token en JSON
